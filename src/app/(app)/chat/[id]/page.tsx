...
export default function ChatPage({ params }: ChatPageProps) {
  ...
  // Find a confirmed (closed) invite — for the "already met" state
  ...
  // Track whether the user sent at least one message this session
  const sentMessageRef = useRef(false);

  // Ref to capture latest values for the cleanup
  const activeInviteRef = useRef(activeInvite);
  const otherUserIdRef = useRef(otherUserId);
  useEffect(() => { activeInviteRef.current = activeInvite; }, [activeInvite]);
  useEffect(() => { otherUserIdRef.current = otherUserId; }, [otherUserId]);

  // Auto-confirm meetup when user navigates away (if they sent at least 1 message)
  useEffect(() => {
    return () => {
      if (sentMessageRef.current && activeInviteRef.current) {
        confirmMeetup(activeInviteRef.current.id, otherUserIdRef.current);
      }
    };
  }, []);
    ...
    return (
      <div className="flex flex-col h-screen bg-gradient-ice overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 pt-3 pb-3 bg-white/80 backdrop-blur-md border-b border-brand-lavender shadow-sm shrink-0">
          <button onClick={() => router.back()} aria-label="返回" className="p-1.5 rounded-full hover:bg-brand-snow active:bg-brand-lavender transition-colors">
            <ArrowLeft className="w-5 h-5 text-brand-ink" />
          </button>
          <div className="relative shrink-0">
            <img src={otherUser?.avatarUrl ?? ''} alt={otherUser?.nickname ?? ''} className="w-9 h-9 rounded-full object-cover border border-brand-lavender" />
          </div>
        ...
        {/* Past messages (read-only) */}
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
          {localMessages.map((msg) => {
            const isMine = msg.senderId === currentUser?.id;
            return (
              <div key={msg.id} className={`flex items-end gap-2 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
                {!isMine && <img src={otherUser?.avatarUrl ?? ''} alt="" className="w-7 h-7 rounded-full object-cover shrink-0 border border-brand-lavender" />}
                <div className={`flex flex-col gap-0.5 max-w-[72%] ${isMine ? 'items-end' : 'items-start'}`}>
                  <div className={`px-3.5 py-2 rounded-2xl text-sm leading-relaxed opacity-60 ${isMine ? 'bg-brand-sky text-brand-ink rounded-br-md' : 'bg-white border border-brand-lavender text-brand-ink rounded-bl-md'}`}>
                    {msg.text}
                  </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-ice overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 pt-3 pb-3 bg-white/80 backdrop-blur-md border-b border-brand-lavender shadow-sm shrink-0">
        ...
        <div className="relative shrink-0">
          <img src={otherUser?.avatarUrl ?? ''} alt={otherUser?.nickname ?? '用戶'} className="w-9 h-9 rounded-full object-cover border border-brand-lavender" />
          {isOtherOnline && <span className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-status-available border-2 border-white" />}
        </div>
        <div className="flex flex-col min-w-0 flex-1">
          <span className="font-semibold text-sm text-brand-ink truncate leading-tight">{otherUser?.nickname ?? otherUserId}</span>
          <span className="text-xs text-zinc-400 leading-tight">{isOtherOnline ? '目前在線' : '離線'}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        ...
        {localMessages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex items-end gap-2 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
              {!isMine && <img src={otherUser?.avatarUrl ?? ''} alt={otherUser?.nickname ?? ''} className="w-7 h-7 rounded-full object-cover shrink-0 self-end border border-brand-lavender" />}
              <div className={`flex flex-col gap-0.5 max-w-[72%] ${isMine ? 'items-end' : 'items-start'}`}>
                <div className={`px-3.5 py-2 rounded-2xl text-sm leading-relaxed ${isMine ? 'bg-brand-sky text-brand-ink rounded-br-md' : 'bg-white border border-brand-lavender text-brand-ink rounded-bl-md'}`}>
                  {msg.text}
                </div>
                <span className="text-[10px] text-zinc-400 px-1">{formatTime(msg.createdAt)}</span>
              </div>
