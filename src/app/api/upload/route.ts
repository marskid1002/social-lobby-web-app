...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body?.action === 'delete') {
      ...
      if (!blobAvailable && !r2Available) return NextResponse.json({ ok: true });

      // 嚴格驗證 URL / host / pathname（不用 includes、不信任 client 任何欄位）
      const parsedUrl = parseStoredImageUrl(url);
      if (!parsedUrl.ok) return NextResponse.json({ error: 'invalid url' }, { status: 400 });

      let authoritativePathname: string | null = parsedUrl.pathname;
      let deleteVercel: ((url: string) => Promise<void>) | null = null;
      // 以 provider head() 取得權威 pathname；找不到 → 冪等（無 del）；其他 provider 錯誤 → 一般化 500
      if (parsedUrl.provider === 'vercel') {
        ...
        return NextResponse.json({ error: '圖片儲存尚未設定' }, { status: 503 });
      }
      ...
      if (parsedUrl.provider === 'r2') await deleteR2Object(parsedUrl.pathname);
      else await deleteVercel!(url);
    ...
    try {
      if (r2Available) {
        const url = await putR2Object(pathname, parsed.buffer, parsed.mime);
      ...
      // 儲存失敗：只記一般訊息，不輸出 token / buffer / base64 / 完整 provider error
      console.error('[upload] image storage put failed');
      // 正式站報錯（dataURL 會被 /api/sync 擋下，不做無效退回）；本地才退回已驗證的 dataURL
      if (isProd) return NextResponse.json({ error: '圖片上傳失敗，請確認圖片儲存設定' }, { status: 502 });
      return NextResponse.json({ url: dataUrl, fallback: true });
    }
  } catch {
